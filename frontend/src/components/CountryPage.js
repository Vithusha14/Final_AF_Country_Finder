...
const Countrypage = ({ countries, loading }) => {
  let { countryName } = useParams(); // Get the country name (or code) from URL
  let navigate = useNavigate();

  console.log("Country Name from URL:", countryName); // Log countryName from URL

  // Find the selected country from the countries list
  let country = countries.find((item) => {
    return item.alpha3Code === countryName; // Use country code for comparison
  });
              ...
              <div className="mt-8 flex items-center flex-wrap">
                ...
                {country["borders"] && country.borders.map((border) => {
                  ...
                  return (
                    <p
                      onClick={() =>
                        navigate(`/country/${bCountry.alpha3Code}`)} /* ✅ updated path */
